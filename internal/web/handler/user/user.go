...
import (
	...
	"github.com/KNICEX/InkFlow/internal/service/code"
	"github.com/KNICEX/InkFlow/internal/web/handler"
	ijwt "github.com/KNICEX/InkFlow/internal/web/jwt"
	"github.com/KNICEX/InkFlow/internal/web/middleware"
...
func NewUserHandler(svc service.UserService,
	emailCodeSvc code.Service,
	jwtHandler ijwt.Handler, auth middleware.Authentication, log logx.Logger) *Handler {
	return &Handler{
		svc:          svc,
...
func (u *Handler) RegisterRoutes(server *gin.RouterGroup) {
	...
	// 登录验证码
	server.POST("/verify/send/login", ginx.WrapBody(u.logger, u.SendLoginCode))

	loginGroup := server.Group("/login")
	{
		// 邮箱验证码登录(自动注册)
		loginGroup.POST("/email", ginx.WrapBody(u.logger, u.LoginEmail))
		// 手机验证码登录(自动注册)
		//loginGroup.POST("/sms", ginx.WrapBody(u.logger, u.LoginSMS))
		// 邮箱密码登录
		loginGroup.POST("/pwd/email", ginx.WrapBody(u.logger, u.LoginEmailPwd))
		// 手机密码登录
		loginGroup.POST("/pwd/phone", ginx.WrapBody(u.logger, u.LoginPhonePwd))
		// 用户名密码登录
		loginGroup.POST("/pwd/account_name", ginx.WrapBody(u.logger, u.LoginPhonePwd))
	}
	...
	checkGroup.Use()
	{
		checkGroup.GET("/logout", u.Logout)

		checkGroup.GET("/profile", u.Profile)
		checkGroup.PUT("/profile", ginx.WrapBody(u.logger, u.EditProfile))
		// 修改账号名
		checkGroup.PUT("/account_name", ginx.WrapBody(u.logger, u.EditAccountName))

		// 发送重置密码验证码
		//checkGroup.POST("/verify/send/reset/sms", ginx.Wrap(u.logger, u.SendResetPwdCodeSms))
		checkGroup.POST("/verify/send/reset/email", ginx.Wrap(u.logger, u.SendResetPwdCodeEmail))

		// 重置密码
		checkGroup.POST("/pwd/reset/email", ginx.WrapBody(u.logger, u.ResetPwdByEmailCode))
		//checkGroup.POST("/pwd/reset/sms", ginx.WrapBody(u.logger, u.ResetPwdBySmsCode))
		checkGroup.POST("/pwd/reset/old", ginx.WrapBody(u.logger, u.ChangePwd))
	}
}
...
func (u *Handler) Logout(ctx *gin.Context) {
	...
	result.SuccessWithMsg(ctx, "登出成功")
}
...
func (u *Handler) Profile(ctx *gin.Context) {
	...
	result.SuccessWithData(ctx, Profile{
		Email:       user.Email,
		Phone:       user.Phone,
...
func (u *Handler) RefreshToken(ctx *gin.Context) {
	...
	token, err := jwt.ParseWithClaims(req.RefreshToken, &rc, func(token *jwt.Token) (any, error) {
		return ijwt.RefreshClaimsKey, nil
	})
	...
	if rc.ExpiresAt.Time.Sub(time.Now()) < time.Hour*24*7 {
		...
		if err = u.ClearToken(ctx); err != nil {
			u.logger.WithCtx(ctx).Error("RefreshToken 一并刷新refreshToken失败", logx.Error(err))
		}
		err = u.SetLoginToken(ctx, domain.User{
			Id: rc.UserId,
		})
	} else {
		// 只刷新短token
		err = u.SetJwtToken(ctx, domain.User{
			Id: rc.UserId,
		}, rc.Ssid)
	}
	if err != nil {
		u.logger.WithCtx(ctx).Error("RefreshToken", logx.Error(err))
		result.InternalError(ctx)
		return
	}
