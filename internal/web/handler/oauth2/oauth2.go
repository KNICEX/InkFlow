...
func (o *oAuth2Handler[T]) AuthUrl(ctx *gin.Context) {
	...
	result.SuccessWithData(ctx, url)
}
...
func (o *oAuth2Handler[T]) Callback(ctx *gin.Context) {
	code := ctx.Query("code")
	if code == "" {
		result.InvalidParam(ctx)
		return
	}
	...
	if err != nil {
		...
		result.InvalidParamWithMsg(ctx, "登录失败")
		return
	}
