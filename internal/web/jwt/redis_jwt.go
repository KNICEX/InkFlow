...
func (h *RedisHandler) SetLoginToken(ctx *gin.Context, user domain.User) error {
	ssid := uuid.New().String()
	if err := h.setRefreshToken(ctx, user, ssid); err != nil {
		return err
	}
	return h.SetJwtToken(ctx, user, ssid)
}

func (h *RedisHandler) SetJwtToken(ctx *gin.Context, user domain.User, ssid string) error {
	claims := UserClaims{
		UserId: user.Id,
		Ssid:   ssid,
...
func (h *RedisHandler) setRefreshToken(ctx *gin.Context, user domain.User, ssid string) error {
	rc := RefreshClaims{
		UserId: user.Id,
		Ssid:   ssid,
		RegisteredClaims: jwt.RegisteredClaims{
